...
export class Note {
   ...
   */
  static playChord(listOfNoteObjects, duration=3, startTime=0) {
    if (Note.audioCtx === undefined) {
      Note.setAudioContext();
    }
    ...
    chord.forEach(source => {
      // source.start(Note.audioCtx.currentTime + delay, 0, duration);
 ...
 */
export function playNotes(playing, duration=.75, ascending=true, sustain=true, when=0) {
  stopAll() // Stop all other music before playing the scale.
    .then( () => {
      // let startTime = Note.audioCtx.currentTime;
      let startTime = when; //0;
...
// New playChordProgression that uses web audio api.
 ...
 * @returns {void} void
 */
 export async function playChordProgression(rootNote, chordProgressionAsSymbols, duration=1.1) {
  if (Note.audioCtx === undefined) {
    Note.setAudioContext();
  }

  // Get random root note if one isn't supplied.
    // Actually no because then this becomes asynchronous.
  // if (rootNote == undefined || !(rootNote instanceof Note)) {console.error(`rootNote is undefined`);rootNote = new Note();}
  if (rootNote == undefined) {
    console.error(`In playChordProgression(): rootNote is undefined.`);
    return;
  }
  
  // Get random chord progression if one isn't supplied.
    // same logic. just don't give bad inputs.
  let chordProgMap = getChordProgessionMap(chordProgressionAsSymbols);
  if (chordProgMap == undefined) {
    // chordProgMap = getChordProgessionMap();
    console.error(`In playChordProgression(): chordProgMap is undefined.`);
    return;
  }
  ...
  let listOfChords = [];
  for (let i = 0; i < chordProgMap.length; i++) {
    let [semitones, symbol] = chordProgMap[i];
    const nextNote = await rootNote.nextNote(semitones);

    // Turn this (note, symbol) 2-tuple into a Note array (chord).
    let chordMap = getChordMapFromSymbol(symbol);
    let newChord = await chordMapToChord(nextNote, chordMap);
    // if (DEBUG) console.log(`playChordProgression(): new chord: ${newChord}, symbol = ${symbol}`);
    listOfChords.push(newChord); 
  }

  // Play the chord progression (and play last chord 3x as long).
  let timeBetweenChords = duration;
  let startTime = Note.audioCtx.currentTime;
  let lastIndex = listOfChords.length-1;
  for (let i = 0; i < lastIndex; i++) {
    const chord = listOfChords[i];
    Note.playChord(chord, timeBetweenChords, startTime);
    startTime += timeBetweenChords;
  }
  Note.playChord(listOfChords[lastIndex], timeBetweenChords*3, startTime);

  // Play the new chord progression.
  // playChordProgressionWrapper(newChordProgression, delay=1100);
}

// Old playChordProgression that used <audio>
// /** Play a chord progression given it's root note and a list of chords.
//  * @param {Note} rootNote - Root note to base the progression on.
//  * @param {Array<Array<number,string>>} chordProgMap - An array of arrays describing chords. Should always be generated by getChordProgessionMap();
//  * @returns {void} void
...
// Todo: not useful anymore, detete.
 ...
 */
 export async function makeScale(rootNote, scaleMap) {
